...
def check_control_events(ai_settings, screen, stats, sb, play_button, u_p_b, u_p_t, u_p_r, sushi_ball):
    # Respond to keypress and mouse events.
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:
            check_keydown_events(event, u_p_b, u_p_t, u_p_r)
        elif event.type == pygame.KEYUP:
            check_keyup_events(event, u_p_b, u_p_t, u_p_r)
        elif event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            check_play_button(ai_settings, screen, stats, sb, play_button,
                              sushi_ball, mouse_x, mouse_y)


# noinspection PyTypeChecker
def check_play_button(ai_settings: object, screen: object, stats: object, sb: object, play_button: object,
                      sushi_ball: object,
                      mouse_x: object,
                      mouse_y: object) -> object:
    ...
    if not stats.game_active:
        ...
        if stats.high_score != 0:
            if stats.user_score > stats.ai_score:
                ...
                winner_greet = menu_font.render(" ", False, (0, 0, 0))
...
def check_keydown_events(event, u_p_b, u_p_t, u_p_r):
...
def check_keyup_events(event, u_p_b, u_p_t, u_p_r):
...
def change_sushi_direction(sushi_pieces):
...
def check_match_events(ai_settings, screen, stats, sb, paddles, sushi_ball):
    ...
    for paddle in paddles:
        if pygame.sprite.spritecollideany(paddle, sushi_ball):
            paddle_hit(ai_settings, stats, paddle, sushi_ball)
